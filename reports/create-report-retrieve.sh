...
_post_api_call_bearer "api/2/reports" \
					"application/json" \
					"${_json}" > "${DIR}/../created_report.json"

# Get report_id from saved file
# Call a function to pipe JSON from file, extract JSON property, remove quotas from the property's value
_report_id=$(_get_id_from_file "${DIR}/../created_report.json")
...
while [[ $_report_status != "saved" ]] ; do
	...
	_get_api_call_bearer "api/2/reports/${_report_id}/stored" \
					  > "${DIR}/../${_report_id}_report_status.json"


	_report_status=$(jq '.items[0].status' < "${DIR}/../${_report_id}_report_status.json" | sed -e 's/^"//' -e 's/"$//')
...
_stored_report_id=$(jq '.items[0].id' < "${DIR}/../${_report_id}_report_status.json" | sed -e 's/^"//' -e 's/"$//')
